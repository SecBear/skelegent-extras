//! Load decision cards from golden repository.
...
/// Parse a comma-separated decisions argument into validated IDs.
///
/// Returns all 23 IDs if `arg` is `None`.
...
pub async fn load_card(
    ...
    let filename = decision_filename(id)
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, format!("unknown decision: {id}")))?;
...
mod tests {
    ...
    fn all_ids_have_filenames() {
        for id in ALL_DECISION_IDS {
            assert!(decision_filename(id).is_some(), "no filename for {id}");
        }
    }
}
