...
impl<P: Provider + 'static> Operator for SynthesisOperator<P> {
    async fn execute(&self, input: OperatorInput) -> Result<OperatorOutput, OperatorError> {
        ...
        let pass1_request = ProviderRequest {
            model: Some(self.config.model.clone()),
            messages: vec![ProviderMessage {
                role: Role::User,
                content: vec![ContentPart::Text { text: verdicts_json }],
            }],
            tools: vec![],
            max_tokens: None,
            temperature: None,
        ...
        for flag in flags.iter().take(self.config.max_deep_dives) {
            ...
            let pass2_request = ProviderRequest {
                model: Some(self.config.model.clone()),
                messages: vec![ProviderMessage {
                    role: Role::User,
                    content: vec![ContentPart::Text {
                        text: "Produce the deep analysis.".to_string(),
                    }],
                }],
                tools: vec![],
                max_tokens: None,
...
mod tests {
    ...
    /// Mock LLM provider that returns `pass1_json` on the first call and
    /// `pass2_json` on every subsequent call.
    ///
    /// `call_count` is incremented before each response so tests can assert
    /// exactly how many passes were executed.
    ...
    impl Provider for CountingProvider {
        ...
        {
            // fetch_add returns the *old* value, so 0 == first call (Pass 1).
            let n = self.call_count.fetch_add(1, Ordering::SeqCst);
            let json = if n == 0 {
                self.pass1_json.clone()
            } else {
                self.pass2_json.clone()
            };
            async move {
                Ok(ProviderResponse {
    ...
    async fn synthesis_operator_skips_low_confidence_flags() {
        ...
        // Pass 2 JSON is irrelevant — it must never be requested.
        let pass2_json = "{}".to_string();
        let call_count = Arc::new(AtomicUsize::new(0));
