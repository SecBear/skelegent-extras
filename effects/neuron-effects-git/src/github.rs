...
impl GitHubApiClient {
    ...
    fn authed_request(
        ...
        builder
            ...
            .header("User-Agent", "neuron-effects-git/0.4.0")
    }
}
...
mod tests {
    ...
    fn test_client() -> GitHubApiClient {
        GitHubApiClient::new("owner", "repo", "NEURON_TEST_GH_TOKEN")
    }
    ...
    fn resolve_token_missing() {
        let client = GitHubApiClient::new("owner", "repo", "NEURON_TEST_GH_TOKEN_MISSING_8f3a2c");
        let err = client.resolve_token().unwrap_err();
        match err {
            GitError::ApiError(msg) => {
                assert!(msg.contains("NEURON_TEST_GH_TOKEN_MISSING_8f3a2c"));
            }
