...
//! OAuth tokens. Use [`neuron-auth-pi`] as the first chain link for providers
...
///
/// Intended as a fallback in an [`neuron_auth::AuthProviderChain`] after
/// [`neuron-auth-pi`]. Handles credentials that are only available in OMP
